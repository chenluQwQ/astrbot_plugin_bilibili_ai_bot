...
class PersonaEngine:
    ...
    async def snapshot(self) -> LifeState:
        ...
        if row:
            return LifeState(
                id=row["id"], at=row["at"], mood=row["mood"],
                energy=row["energy"], social=row["social"], phase=row["phase"],
                note=row["note"],
    ...
    async def _init_state(self) -> LifeState:
        state_id = await self._db.execute(
            ...
            "VALUES(?,?,?,?,?,?,?,?)",
            (now(), "calm", 0.8, 0.3, "idle", "初始化",
             json.dumps({}), json.dumps({"tokens_daily": self._token_budget_daily()})),
        )
        return LifeState(
            id=state_id, at=now(), mood="calm", energy=0.8, social=0.3,
            phase="idle", note="初始化", interests={},
    ...
    async def update_state(
        ...
        state_id = await self._db.execute(
            "INSERT INTO life_states(at,mood,energy,social,phase,note,interests,budget) "
            "VALUES(?,?,?,?,?,?,?,?)",
            (now(), new_mood, new_energy, new_social, new_phase, note,
             json.dumps(prev.interests), json.dumps(prev.budget)),
        )
        return LifeState(
            id=state_id, at=now(), mood=new_mood, energy=new_energy,
            social=new_social, phase=new_phase, note=note,
            interests=prev.interests, budget=prev.budget,
        )
    ...
    async def today_segments(self, date_key: str | None = None) -> list[DayPlanSegment]:
        ...
        return [
            DayPlanSegment(
                ...
                activity=row["activity"], intent=row["intent"],
                energy_cost=row["energy_cost"], planned_by=row["planned_by"],
    ...
    async def _generate_plan(self, date_key: str) -> None:
        ...
        segments = [
            ...
            (time_to_min(time(9, 0)), time_to_min(time(12, 0)), "reply", "上午活跃时段"),
            (time_to_min(time(12, 0)), time_to_min(time(14, 0)), "rest", "午休"),
            (time_to_min(time(14, 0)), time_to_min(time(18, 0)), "social", "下午社交时段"),
            (time_to_min(time(18, 0)), time_to_min(time(20, 0)), "create", "晚间整理"),
            (time_to_min(time(20, 0)), time_to_min(time(22, 0)), "browse", "夜间轻松互动"),
        ...
        if sleep_start > sleep_end:
            ...
            segments.append((time_to_min(sleep_start), time_to_min(sleep_end), "rest", "夜间睡眠"))

        for start_min, end_min, activity, intent in segments:
            await self._db.execute(
                "INSERT INTO day_plans(plan_date,start_min,end_min,activity,intent,"
                "energy_cost,planned_by,state,summary,stats,created_at) "
                "VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                (date_key, start_min, end_min, activity, intent,
                 0.1 if activity == "rest" else 0.3, "bot", "planned", "",
                 json.dumps({}), now()),
            )
    ...
    async def should_respond(self, priority: int) -> tuple[bool, str]:
        ...
        if seg.activity == "rest" and priority < 90:
    ...
    async def adjust_relationship(
        ...
        if familiarity_delta != 0.0:
            updates["familiarity"] = max(0.0, min(1.0, profile.familiarity + familiarity_delta))
        if trust_delta != 0.0:
