...
本仓库是基于原作者 [chenluQwQ/astrbot_plugin_bilibili_ai_bot](https://github.com/chenluQwQ/astrbot_plugin_bilibili_ai_bot) 的社区 Fork 版本。特别感谢原作者持续维护 BiliBot，并为 AstrBot 生态提供了完整的 B 站 AI Bot 基础能力。我们会尽量保持与上游设计和配置的兼容；如需使用原始版本或查看上游最新变更，请优先参考原作者仓库。

在继承原版评论互动、私信、直播弹幕、主动行为、记忆和人格等能力的基础上，本版本主要增加和改进了以下内容：

- **四层运行时架构**：将事件适配、安全治理、存储、媒体理解、人格和调度等职责拆分，减少模块之间的隐式耦合，便于定位问题和继续扩展。
- **统一事件与状态流转**：引入统一事件模型、SQLite 状态记录、事件领取与状态审计，并保留旧事件适配器兼容入口，降低升级成本。
- **更完整的安全边界**：增加调用者身份 namespace、会话与记忆隔离、工具分级和一次性写操作能力控制，降低跨用户数据污染和误操作风险。
- **内置 WebUI 控制中心**：新增 `pages/bilibot/` 页面，集中管理运行状态、配置、账号登录、扫码登录、主动行为、记忆、媒体缓存和安全能力，减少手动编辑配置文件的需要。
- **可观测性与可维护性提升**：统一 SQLite 存储、事件统计、缓存治理和周期任务入口，让 Bot 的运行状态更容易查看、调试和恢复。
- **更舒适的管理体验**：管理页面采用响应式布局、清晰的分区和状态反馈，适合在桌面端与较窄窗口中使用。

这些改动的目标不是改变原版 BiliBot 的使用方式，而是在保留原有能力的基础上，让项目更安全、更容易维护，也更方便后续接入新的平台事件、工具和管理功能。
